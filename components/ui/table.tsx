...
function TableHeader({ className, ...props }: React.ComponentProps<'thead'>) {
  return (
    <thead
      ...
      className={cn(
        'sticky top-0 z-10 bg-muted/70 backdrop-blur supports-[backdrop-filter]:bg-muted/60 [&_tr]:border-b',
        className,
      )}
      {...props}
    />
...
function TableRow({ className, ...props }: React.ComponentProps<'tr'>) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        'hover:bg-accent/35 data-[state=selected]:bg-muted border-b border-border/50 transition-colors duration-150',
        className,
      )}
      {...props}
    />
